...
if __name__ == "__main__":
            ...
            if interaction.user == self.user1:
                ...
                if duelrating is not None and (duelrating < 800 or duelrating > 3500 or duelrating % 100 != 0):
                    ...
                    cur.execute('Insert into duelchallenge values(%s,%s,%s,%s,%s)',
                                (str(self.inter.user.id), str(interaction.user.id), unsolgiven[0], unsolgiven[1],
                                 now1[0],))
                    conn.commit()
    ...
    class ButtonYesNoduel_end(View):
        ...
        async def no(self, interaction, button):
            if str(interaction.user.id) == str(self.user1):
                await self.inter.edit_original_response(
                    embed=send_embed("Draw Declined",
                                     "Looks like your opponent has some ideas up his sleeve. Buckle up"),
                    view=None)
            else:
                await interaction.edit_original_response('Tere lie nahi tha bhai', ephemeral=True)
    ...
    async def solo_end(Interaction: discord.Interaction):
        ...
        if booea == 1:
            ...
            cur.execute("UPDATE SoloLeaderboard set score = %s where DiscordId = %s",
                        (newscore, str(Interaction.user.id),))
            conn.commit()
            t2 = rows[0][4]
    ...
    async def handle_set(Interaction: discord.Interaction, disc_id: discord.Member, handle: str):
        ...
        if response.status_code == 200:
            ...
            await Interaction.edit_original_response(
                embed=send_embed("Handle Identified",
                                 f"Handle Identified Successfully for {disc_id.mention} to {handle}"))
        else:
    ...
    async def duel_challenge(Interaction: discord.Interaction, handle_name: discord.Member, rating: int = None):
        ...
        if not rows:
            await Interaction.edit_original_response(
                embed=send_embed("Handle not identified",
                                 f"Looks like {handle_name.mention} has not identified handle"))
            return
        if duelrating is not None and (duelrating < 800 or duelrating > 3500 or duelrating % 100 != 0):
            await Interaction.edit_original_response("Rating dhang se daal")
            return
        ...
        await Interaction.edit_original_response(embed=send_embed(ti, desc),
                                                 view=ButtonYesNo(Interaction, 30, handle_name))
    ...
    async def graph_compare(Interaction: discord.Interaction, p1: str):
        ...
        for handle in mainlist:
            ...
            for dates in details_api:
                date.append(
                    datetime.strptime(unixTimeToHumanReadableVaibhav(dates["ratingUpdateTimeSeconds"]), "%Y-%m-%d"))
